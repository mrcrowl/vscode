...
export default class TypeScriptCompileOnSaveSupport {
	...
	public onDidSaveTextDocument(document: TextDocument) {
		...
		this.client.execute('compileOnSaveAffectedFileList', affectedFileArgs).then((response: any) => {
			const { body } = response;
			...
			for (let file of affectedFiles) {
				...
				if (emit) {
					...
					this.client.execute('compileOnSaveEmitFile', emitArgs).then((_: any) => {
						if (batch) {
...
class CompileOnSaveMultipleFileBatcher {
	...
	public compileMessage: Disposable | null = null;

	private creationTimestamp: number;
	private firstDidSaveTimestamp: number | null = null;
	private pendingSavedDocuments: Map<string, boolean>;
	private affectedFilesStatusHash: Map<string, AffectedFileStatus>;
	private affectedFilesRemainingEmitCount: number = Infinity;
